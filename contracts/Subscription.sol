...
contract Subscription {
     ...
     * @param encPayment   Client-side encrypted payment amount (must equal encrypted allocation).
     * @param paymentProof ZKPoK proof generated by the relayer SDK.
     *
     * @dev  The investor encrypts their own allocation amount (which they know from
     *       decrypting it via the relayer SDK) and submits it here.
     *       TFHE.eq validates equality without revealing either value.
     *       The transfer amount is the investor-provided encrypted value, which the
     *       FHE gate guarantees equals the founder-set allocation.
     */
    function subscribe(
        uint256 roundId,
        einput encPayment,
        ...
        euint64 payment = TFHE.asEuint64(encPayment, paymentProof);

        // FHE equality gate — transaction reverts on-chain if payment != allocation
        // Neither value is revealed; the proof is purely in ciphertext space
        ebool valid = TFHE.eq(alloc, payment);
        TFHE.optReq(valid);

        // Encrypted transferFrom: amount stays private, moves investor → escrow
        // Investor must have called cUSDT.approve(subscriptionContract, encAllocation) first
        cUSDT.transferFrom(msg.sender, address(this), payment);

        // State updates
        allocations.markSubscribed(roundId, msg.sender);
        equity.mint(msg.sender, roundId, 0); // amount=0: cEquity mirrors allocation via FHE balance
    ...
    function founderWithdraw(uint256 roundId, address to) external {
        ...
        TFHE.allowTransient(bal, address(cUSDT));
